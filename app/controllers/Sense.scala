...
final class Sense(env: Env)(implicit mat: akka.stream.Materializer) extends LilaController(env) {

  def home =
    Open { implicit ctx =>
      NoBot {
        Ok(html.sense.home).fuccess
      }
    }
  ...
  def create =
    WithPlayerId { implicit ctx => playerId =>
      env.racer.api.createAndJoin(playerId) map { raceId =>
        Redirect(routes.Sense.show(raceId.value))
      }
    }
  ...
  def show(id: String) =
    WithPlayerId { implicit ctx => playerId =>
      env.racer.api.get(RacerRace.Id(id)) match {
        case None => Redirect(routes.Sense.home).fuccess
        case Some(r) =>
      ...
      env.racer.api.get(RacerRace.Id(id)) match {
        ...
        case Some(race) =>
          env.racer.api.rematch(race, playerId) map { rematchId =>
            Redirect(routes.Sense.show(rematchId.value))
          }
      }
    }
